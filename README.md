...
We add some typings for our tasks and our apis request/response
...
To test out our project locally, we can do:

```bash
yarn dev
```

This will start the nitric api with `nitric run` and the nextjs server `next dev`.

If you want to test just the backend add to your package.json scripts:

```json
"nitric-run": "nitric run \"/apis/todo-api/functions/*.ts\" --verbose 2"
```

and run `yarn nitric-run`
...
> When you're done, you can destroy the stack with `nitric stack down`
